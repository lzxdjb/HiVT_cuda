...
)
{
    ...
    else
    {
        ...
        for (int i = edge_begin; i < edge_end; i++)
        {
            ...
            float r_temp_0 = rotate_mat[edge_index_0[i] % x_size * 4 ];
            float r_temp_1 = rotate_mat[edge_index_0[i] % x_size * 4 + 1];
            float r_temp_2 = rotate_mat[edge_index_0[i] % x_size * 4 + 2];
            float r_temp_3 = rotate_mat[edge_index_0[i] % x_size * 4 + 3];

            bmm1[0] = temp_0 * r_temp_0;
            bmm1[0] += temp_1 * r_temp_2;
            bmm1[1] = temp_0 * r_temp_1;
            bmm1[1] += temp_1 * r_temp_3;

            bmm2[0] = edge_ati_attr[i * 2] * r_temp_0 + edge_ati_attr[i * 2 + 1] * r_temp_2;
            bmm2[1] = edge_ati_attr[i * 2] * r_temp_1 + edge_ati_attr[i * 2 + 1] * r_temp_3;
