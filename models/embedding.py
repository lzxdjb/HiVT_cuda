...
class MultipleInputEmbedding(nn.Module):
    ...
    def forward(self,
        ...
        for i, v in enumerate(self.module_list):
            # print("continuous_inputs[i]",continuous_inputs[i].shape)
            # print("v(continuous_inputs[i])",v(continuous_inputs[i]).shape)
        ...
        if categorical_inputs is not None:
            # print("fuck!!!")
            # print(categorical_inputs[0].shape)
            # print(categorical_inputs[1].shape)
            # print(categorical_inputs[2].shape)

            output += torch.stack(categorical_inputs).sum(dim=0)
        # print("after",output.shape)
        # print("output",self.aggr_embed(output))
